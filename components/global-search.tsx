...
export function GlobalSearch() {
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative z-50 min-w-0 transition-all duration-300 ease-in-out",
        isExpanded ? "w-full max-w-full sm:w-[420px] lg:w-[640px]" : "w-full min-w-0 sm:w-56 lg:w-72",
      )}
    >
      <Command
        shouldFilter={false}
        ...
        <AnimatePresence>
          {showDropdown && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="absolute left-0 right-0 top-[calc(100%+0.5rem)] flex max-h-[min(70vh,460px)] flex-col overflow-hidden rounded-xl border border-border/80 bg-card shadow-2xl shadow-black/5 sm:rounded-2xl"
            >
              <div className="flex items-start justify-between gap-3 border-b border-border/60 bg-muted/30 px-3 py-3 sm:px-4">
                <div className="min-w-0">
                  {/* ФІКС 5: Красиві заголовки для порожнього стану */}
              ...
              <CommandList className="flex-1 max-h-[min(56vh,380px)] px-1.5 py-1.5 sm:px-2 sm:py-2">
                ...
                {/* 3. Головний фікс: прибрали !isLoading звідси! */}
                {/* Тепер результати відобразяться миттєво, як тільки будуть знайдені */}
                {totalResults > 0 && (
                  <>
